...
{  // cerr<<"func: "<<query_start<<' '<<last<<endl;
    ...
    while(a[x].first <last){
        ...
        fgets(s,400001,f);
        // if(a[x].first < prev)cerr<<"error "<< prev<< ' '<<a[x].first<<endl;;
        // prev = a[x].first;
        result[x]=s;
        x++;
        if(x%100000==0)cerr<<"looked up "<<x<<endl;
...
void bpt_initialize(int forward = 1, int reverse = 1){
...
void reverse_lookup(int input, string &output){
    char buf[10];
    ll rlen=strlen(reverse_directory);                  //length of path of reverse directory
    strcpy(argv1_copy,data_directory);
    ll tot_db;                                          //total number of databases
    cerr<<"entered\n";
    strcpy(argv1_copy,reverse_directory);
    freopen(lineDB_directory,"r",stdin);                //file_rev has starting strings of each db
    cerr<<"opened linesDB\n";
    cin>>tot_db;
    ll hunny=0;
    ...
    for(ll i=0;i<tot_db;i++){
        //cin>>linesDB[i];
        cin>>tempLines;
        if(tempLines + totalLines >= input){
            sprintf(buf,"%lld",i);
            argv1_copy[rlen]='\0';
            strcat(argv1_copy,buf);
    //      cerr<<"here\n";
            dbr[i]=new bpt::bplus_tree(argv1_copy);
            dbID = i;
            break;
        }
        else{
...
//      cerr<<ind<<' '<<num<<endl;
        ...
        // for (auto x: a)cerr<<"holo "<<x.first<<' '<<x.second<<",";cerr<<endl;
        if(cnt%100000==0){
            cout<<"done "<<cnt<<endl;
        }
        cnt++;
    ...
    cerr<<"lookups for strings begin\n";
...
void print_reverse_output(vector<int> in, vector<string> out){
    return ;
    for(int i=0; i<in.size(); i++)cerr<<in[i]<<' '<<out[i];
    cout<<"batch done\n";
}

int buffer_size = 10000;    //number of reverse lookups at a time

void read_from_pipe (int file)
{ 
      int lookup;
      int tot_sz = 0, cur_sz = 0;
      int batch = 0;
      vector<string> out;
      vector<int> in;
      FILE *stream;
      stream = fdopen (file, "r");
      while (fscanf(stream, "%d", &lookup) != EOF){
        // time(&timev[tot]);
        in.push_back(lookup);
        cur_sz++;
        cerr<<cur_sz<<endl;
        if(cur_sz >= buffer_size){
            // for (auto x:in)cerr<<"got "<<x<<' ';cerr<<endl;
            reverse_lookup(in, out);
            print_reverse_output(in, out);
            out.clear();
            in.clear();
            tot_sz += cur_sz;
            cur_sz = 0;
            batch++;
            cerr<<"batch "<<batch<<" done\n";
        }
...
int main(int argc, char *argv[]){
    printf("%s %s\n", argv[0], argv[1]);
    if (strcmp(argv[1],"1") == 0){   //1 for piping
        bpt_initialize(0, 1);
        cout<<"bpt_initialize over\n";
        read_from_pipe(atoi(argv[0]));
