...
const ll sz=100000000;		//max input size for each database
const int num_thr=1;
ll tot_sz;

void build(char *db_forward,char *db_reverse,ll cur, char *s, FILE* f,ll seek){

	ll temp=0,cnt=1,ignore=0,length=0;
	
    bpt::key_t start,start2;

    dbf[cur]=new bpt::bplus_tree(db_forward, true);
    dbr[cur]=new bpt::bplus_tree(db_reverse, true);
...
void func(ll seek,char *file,char *db_forward,char *db_reverse,ll cur, ll total, ll num){
    ...
    for(ll i=0;i<total;i++){
		...
		build(dbNameForward,dbNameReverse,cur+i,s,f,seek+i*sz);
