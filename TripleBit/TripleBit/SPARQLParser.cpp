      ...
      if (token==SPARQLLexer::LCurly) {
         ...
         if ((token==SPARQLLexer::Identifier)&&(lexer.isKeyword("filter"))) {
            ...
            parseFilter(group,localVars);
        	 ...
        	 if (token==SPARQLLexer::LCurly){
        		 PatternGroup subGroup;
        		 //cout<<"found "<<&subGroup<<endl;
        		 parseGroupGraphPattern(subGroup);
        		 group.optional.push_back(subGroup);
        	 }
          }
         else {
            lexer.unget(token);
...
void SPARQLParser::dfs(PatternGroup& group,long long myID=0){
	  printf("\n--- Group ---\n" );
	         printf("\nList of patterns\n\n");
	         int cnt=0;
	         for (auto i: group.patterns){
	             ...
	             cout<<"Predicate - \n"<<"type -"<<i.predicate.type<<"\nValue -"<<i.predicate.value<<"\nid -"<<i.predicate.id<<"\n\n";
	         ...
	         for (auto i: group.filters){
	          cout<<"Filter #"<<(++cnt)<<endl;
	       	  std::cout<<"\n"<<i.type<<"\n";
	       	  for (auto j: i.values){
	       		  std::cout<<j.type<<" "<<j.value<<"\n";
	       	  }
	         ...
	         printf("\nList of optionals\n");
	         for(int j=0;j<group.optional.size();j++){
	        	 printf("%p\n",&(group.optional)[j]);
	         }
	         cout<<endl<<endl;
