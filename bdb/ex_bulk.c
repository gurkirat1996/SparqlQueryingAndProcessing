 ...
 *
 * Database file:
 *	ex_bulk.db (with multiple get_first_strsub-database support)
 *	
 *	subdbs:	primary:
...
#define	WORDLIST	"../../test/tcl/all_data_uniq_objs_sorted"
long long int DATALEN;			/* The primary value's length */
...
int	bulk_insert(DB_ENV *, DB *, long long int, long long int, long long int, long long *, long long int *, long long int);
...
main(argc, argv)
	...
	char *argv[];
{
	DATALEN=20;			/* The primary value's length */
	STRINGLEN =	(DATALEN - sizeof(int));
...
bulk_insert(dbenv, dbp, numkeys, dups, batchsize, itemcountp, batchcountp, verbose)
	...
	long long int numkeys;		/* The number of distinct keys. */
	long long int dups;		/* The number of additional records per key. */
	long long int batchsize;		/* Store this many items in the bulk DBT. */
	long long *itemcountp;	/* out: the number of items added to the DBT. */
	long long int *batchcountp;	/* out: the number of DB->put() calls done. */
	long long int verbose;		/* If set, print each key to be deleted. */
{
	...
	long long int batchcount;		/* The number of batches performed. */
	long long int pendingitems;	/* The number of items in the current batch. */
	long long int totalitems;		/* The total number of items put into batches. */
	long long int prikey;		/* The current primary key: 0..numkeys-1. */
	...
	FILE *data_file = fopen(WORDLIST,"r");
	prikey=0;
	while(1==1) {
		if(prikey%10000==0)printf("%lld\n",prikey);
		prikey++;

		if(fgets(data_val.str,sizeof(data_val.str),data_file)==NULL)
			break;
		DATALEN = strlen(data_val.str) - 1;
		/*
