...
ex_btrec()
{
	...
	long long int cnt=0;		/* The count variable to read records from wordlist. */
	int ret;		/* Return code from call into Berkeley DB. */
	char *p;		/* Pointer to store buffer. */
	char *t;		/* Pointer to store reverse buffer. */
	char buf[377430];		/* Buffer to store key value. */
	char rbuf[377430];	/* Reverse buffer to store data value. */
	...
	// 	(void)sprintf(buf, "%011lld_", cnt);
	// 	if (fgets(buf + 11, sizeof(buf) - 11, fp) == NULL)
	// 		break;
	 ...
	 */
	FILE *query = fopen(QUERY,"r");
	cnt=0;
	struct timeval begin, end;
	gettimeofday(&begin, NULL);
	while(fgets(buf, sizeof(buf), query) != NULL) {
		if(cnt%10000==0)
			printf("%lld\n",cnt);
		/* Get a record number. */
		printf("recno #> ");
