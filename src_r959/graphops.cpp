...
////----------------givevoid forward_lookup(vector <string>input,vector <ull> &output); proper path here------------
#include "../TripleBit/TripleBit/SPARQLParser.h"
#include "../TripleBit/TripleBit/SPARQLLexer.h"
...
//--------------------B+ tree functions------------------------------------------------------------------------------
void forward_lookup(vector <string>input,vector <unsigned long long> &output);
void reverse_lookup(vector <unsigned long long>input,vector <string> &output);
//-------------------------------------------------------------------------------------------------------------------
...
void query_to_string(string& queryStr, const char* filename)
{
	printf("%s\n",filename);
	ifstream f;
	f.open(filename);
...
void dfs_join(SPARQLParser::PatternGroup& group){
	for (auto i: group.patterns){
		for(auto j: {i.subject, i.object, i.predicate}){
			//cout<<j.type<<' '<<j.value<<endl;
			if(j.type == SPARQLParser::Element::Variable)join[j.id]++;
		}
	}
...
map<int, int> val;
int neg=-1, neg2=-1;//MAX_JVARS-1; //to assign join/non-join values
...
void populate_array(SPARQLParser::PatternGroup& group, string strlevel){

	JVAR *jvar; //jvar interrelated??
	for (auto i: group.patterns){
		...
		auto tmp = i.subject;
		if(tmp.type==SPARQLParser::Element::Variable)tp->sub=assign(tmp.id);
		else{
			vector<unsigned long long> v = {};
			forward_lookup(vector<string> {tmp.value}, v);
			tp->sub = v[0];	//query b+ tree for id;
		} 

		tmp=i.object;
		if(tmp.type==SPARQLParser::Element::Variable)tp->obj=assign(tmp.id);
		else{
			vector<unsigned long long> v;
			forward_lookup(vector<string> {tmp.value}, v);
			tp->obj = v[0];	//query b+ tree for id;
		} 

		tmp=i.predicate;
		if(tmp.type==SPARQLParser::Element::Variable)tp->pred=assign(tmp.id);
		else{
			vector<unsigned long long> v;
			forward_lookup(vector<string> {tmp.value}, v);
			tp->pred = v[0];	//query b+ tree for id;
		}
		...
		int _id;	//to keep value of tp.sub/obj/pred
		for(auto j: {i.subject, i.object, i.predicate}){
			bool found = false;
			if(tm==0)_id=tp->sub;
			else if(tm==1) _id=tp->obj;
			else if(tm==2) _id=tp->pred;
			tm++;
	...
	int len=strlevel.size();
	string strlevel2=strlevel;
	for (auto i: group.optional){
		strlevel2 += cn++;
		populate_array(i, strlevel2 );
...
// void new_required_function(char *filename){
// 	string q;
// 	query_to_string(q,filename);
// 	SPARQLLexer *lexer = new SPARQLLexer(q);
...
{	
	string q;
	query_to_string(q,fname);
	SPARQLLexer *lexer = new SPARQLLexer(q);
	SPARQLParser *parser = new SPARQLParser(*lexer);
	parser->parse();

	//gets hw many times each variable occurs
	dfs_join(parser->patterns);
	tplist_size = 0, jvarlist_size = 0, varlist_size = 0;
	populate_array(parser->patterns, "a");
